# 👋 Hey, I'm Omar

**Cyber Security Student @ Alexandria University** 🎓  
*Passionate about Network Defense • Secure Coding • Cloud Security* 🔐

<p align="center">
  <a href="https://www.linkedin.com/in/omar-fathy-0321183b7">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn">
  </a>
  <a href="mailto:omarfvthy@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email">
  </a>
  <a href="https://github.com/0mar-11">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub">
  </a>
</p>

<p align="center">
  <img src="https://media.giphy.com/media/3o7btNa0RUYa5E7iiQ/giphy.gif" width="600" alt="Cyber Security Animation">
</p>

---

## 👨‍ About Me

- 🎓 2nd Year Student at **Faculty of Computers & Data Science, Alexandria University**
- 🔐 Major: **Cyber Security**
- 🎯 Seeking **Summer Internship** in SOC, Pentesting, or Cloud Security
- 🛠️ Tech Stack: `Python` `Java` `PHP` `MySQL` `Linux` `AWS`

---

## 🚀 Featured Projects

### 🔐 Cryptography Toolkit | Python
**Apr 2026**  
CLI tool implementing encoding, hashing (SHA-256), and symmetric encryption (AES)
- Applied cryptographic concepts for secure data handling
- Implemented multiple encryption algorithms
🔗 [github.com/0mar-11/crypto-proj](https://github.com/0mar-11/crypto-proj)

### 🗄️ University Management System | Java, OOP
**2025**  
Console-based system using custom Linked List data structures
- CRUD operations for students and courses
- Sorting algorithms and validation logic
🔗 [github.com/0mar-11/university-system](https://github.com/0mar-11/university-system)

### 📚 Library Management System | PHP, MySQL
**2026**  
Full-stack web application with secure login
- Relational database design (1NF-3NF)
- SQL Injection prevention with prepared statements
🔗 [github.com/0mar-11/library-system](https://github.com/0mar-11/library-system)

### 📧 Email Management System | PHP, MySQL
**2026**  
Web-based email system with authentication
- Secure user registration and login
- Input validation and secure coding
🔗 [github.com/0mar-11/email-system](https://github.com/0mar-11/email-system)

### 🤖 Maze Solver AI | Python
**2025**  
AI agent using BFS/DFS pathfinding algorithms
- Visualized pathfinding process
- Optimized for shortest path
🔗 [github.com/0mar-11/maze-solver](https://github.com/0mar-11/Maze-Solver-AI)

### 🐧 Linux Automation Scripts | Bash, Kali
**2026**  
Bash scripts for system administration
- User management and log monitoring
- File permission checks
🔗 [github.com/0mar-11/bash-scripts](https://github.com/0mar-11/kali-bash-scripts)

---

## 🛠️ Technical Skills

### Security & Cloud
- **Tools:** Kali Linux, Wireshark, Nmap, Burp Suite
- **Concepts:** Cryptography, OWASP Basics, Network Security, Vulnerability Assessment
- **Cloud:** AWS Cloud Practitioner Certified

### Programming Languages
- **Python** - Automation, Cryptography, AI/ML
- **Java** - OOP, Data Structures
- **C** - System Programming
- **PHP** - Web Development
- **SQL** - Database Queries
- **Bash** - Linux Scripting

### Databases & Tools
- **MySQL** - DDL/DML, Relational Design, Normalization
- **Git/GitHub** - Version Control
- **VS Code, Postman** - Development Tools

---

## 📜 Certifications

### 🏆 AWS Certified Cloud Practitioner
**Amazon Web Services** | 2026  
- Cloud concepts, security, architecture, and pricing
- Shared responsibility model
🔗 [Verify Credential](https://www.credly.com/users/omar-fathy)

---

## 📚 Education

**B.Sc. in Computer Science & Cyber Security**  
Alexandria University, Faculty of Computers & Data Science  
Expected Graduation: **2028** | GPA: **2.77/4.00**

**Relevant Coursework:**  
Network Security • Cryptography • Data Structures • OOP • Database Systems • Operating Systems • Linux Administration

---

## 🌐 Languages

- **Arabic:** Native
- **English:** Intermediate (B1/B2)

---

##  Currently Learning

- [ ] Penetration Testing methodologies
- [ ] SIEM tools (Wazuh, Splunk)
- [ ] CTF challenges on TryHackMe
- [ ] Advanced Network Security
- [ ] ## 🐛 Bug Hunting & Security Research
- 🎯 **Platforms:** HackerOne, Bugcrowd, Intigriti (Active Learner)
- 🔍 **Focus Areas:** Web App Security, OWASP Top 10, IDOR, XSS, SQLi, SSRF
- 🛠️ **Tools:** Burp Suite (Community/Pro), OWASP ZAP, ffuf, Nuclei, subfinder, httpx
- 📚 **Practice & Labs:** PortSwigger Web Security Academy, Hacker101, TryHackMe (Web Fundamentals)
- 📝 **Methodology:** Reconnaissance → Enumeration → Vulnerability Discovery → Validation → Responsible Disclosure
- 📊 **Progress:** Solving labs daily, writing practice reports, and tracking vulnerability patterns

---

## 📬 Let's Connect

I'm actively seeking **Summer 2026 Internship** opportunities!

If you're hiring or know of opportunities in:
- 🛡️ SOC / Incident Response
- 🔓 Penetration Testing
- 🌐 Network Security
- 💻 Application Security

👉 Feel free to reach out via [LinkedIn](https://www.linkedin.com/in/omar-fathy-0321183b7) or [Email](mailto:omarfvthy@gmail.com)!

---

<p align="center">
  <sub>Built with ❤️ and lots of ☕ | "Stay curious, stay secure" 🔐</sub>
</p>
